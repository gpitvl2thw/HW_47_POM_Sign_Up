...
public class No010CheckTable3 {
	...
	public String[][] No010CheckTable3ToS2D(String strNP) {
		...
		String nameElem = No000ValuesAndMetods.getAllElements(
				noElemAllElements, strNP);
		String nameElemLook = nameElem + "_look";
		String nameElemFile = No000ValuesAndMetods.getPropertyes(nameElem,
				strNP);

		WebElement element = null;
		if (No000ValuesAndMetods.getPropertyes(nameElemLook, strNP).equals(
				"xpath")) {
			element = dr.findElement(By.xpath(nameElemFile));
		} else if (No000ValuesAndMetods.getPropertyes(nameElemLook, strNP)
				.equals("id")) {
		...
		for (int i = 0;; i++) {
			try {
				...
				nElNew = No000ValuesAndMetods.getPropertyes(nElOld, strNP); // id_f_title
				nElNewLook = No000ValuesAndMetods.getPropertyes(nElOld
						+ "_look", strNP); // table2_1_look
				...
				try {
					...
					// locate - #3 - (+2)
					s2d[i * amount + 2 + smew][1] = "Locate of "
							+ nElNew
							+ " is "
							+ No000ValuesAndMetods.getPropertyes(nElOld
									+ "_locate", strNP);
					s2d[i * amount + 2 + smew][2] = "Locate of " + nElNew
							+ " is " + element.getLocation();

					// size - #4 - (+3)
					s2d[i * amount + 3 + smew][1] = "Size of "
							+ nElNew
							+ " is "
							+ No000ValuesAndMetods.getPropertyes(nElOld
									+ "_size", strNP);
					...
					// Value - #5 - (+4)
					try {
						nElNewType = No000ValuesAndMetods.getPropertyes(nElOld
								+ "_type", strNP); // val
					} catch (Exception e) {
						nElNewType = "";
					}
					...
					try {
						nElNewVar = No000ValuesAndMetods.getPropertyes(nElOld
								+ "_val", strNP);
					} catch (Exception e) {
						nElNewVar = "";
					}
		...
		s2d[0][1] = "Number of elements in " + nameElem + " is "
				+ No000ValuesAndMetods.getPropertyes(nameElem + "_0", strNP);
		s2d[0][2] = "Number of elements in " + nameElem + " is " + countEl;

		// проверка
		if (!No000ValuesAndMetods.getContinueTC()) {
			No000ValuesAndMetods.setContinueTC((s2d[0][2].equals(s2d[0][1])));
			No000ValuesAndMetods.setContinueErr("ERROR IN - No6CheckTable2");
		}
